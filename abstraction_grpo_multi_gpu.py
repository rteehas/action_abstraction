...
# SLURM typically sets this to the allocated devices (e.g. "0,1,2,3")
_ALLOC = os.environ.get("CUDA_VISIBLE_DEVICES", "")
_GPU_LIST = [x for x in _ALLOC.split(",") if x != ""]

TRAIN_GPU = _GPU_LIST[0] if _GPU_LIST else "0"
INFER_GPUS = _GPU_LIST[1:] if len(_GPU_LIST) > 1 else []  # must be non-empty for multi-GPU inference
...
def _vllm_solver_worker(req_q, resp_q, model_name, gpu_id, gpu_mem_util,
                        sp_solve_kwargs, sp_judge_kwargs, judge_lora_path):
    import os
    ...
    # LoRARequest(name, int_id, path) :contentReference[oaicite:1]{index=1}
...
class MultiSolverClient:
    ...
    def _run(self, kind, texts):
        n = len(texts)
        if n == 0:
            return []

        # Heuristic: keep batching efficiency for small n
        if self.k == 1 or n < 2 * self.k:
            wid = self._rr % self.k
            self._rr += 1
            self.req_qs[wid].put({"kind": kind, "texts": texts, "idxs": list(range(n))})
            msg = self.resp_qs[wid].get()
            return msg["outs"]

        buckets = [[] for _ in range(self.k)]
        idx_buckets = [[] for _ in range(self.k)]
        for i, t in enumerate(texts):
            wid = i % self.k
            buckets[wid].append(t)
            idx_buckets[wid].append(i)

        pending = []
        for wid in range(self.k):
            if buckets[wid]:
                self.req_qs[wid].put({"kind": kind, "texts": buckets[wid], "idxs": idx_buckets[wid]})
                pending.append(wid)

        out = [None] * n
        for wid in pending:
            msg = self.resp_qs[wid].get()
            for i, s in zip(msg["idxs"], msg["outs"]):
                out[i] = s
        return out

    def generate_texts(self, texts):
        return self._run("solve", texts)

    def judge_texts(self, texts):
        return self._run("judge", texts)
...
if __name__ == "__main__":
    ...
    if len(INFER_GPUS) == 0:
        raise RuntimeError("Need at least 2 GPUs: 1 train + >=1 inference")

    print(f"TRAINING WITH GPU {TRAIN_GPU}")
    print(f"INFERENCING WITH GPUs {INFER_GPUS}")    
    ...
    req_qs, resp_qs, solver_procs = [], [], []
    for gpu_id in INFER_GPUS:
        rq = ctx.Queue()
        pq = ctx.Queue()
        p = ctx.Process(
            target=_vllm_solver_worker,
            args=(rq, pq, MODEL_NAME, gpu_id, 0.5, sp_solve_kwargs, sp_judge_kwargs, binary_judge_lora_path),
        )
        p.start()
        req_qs.append(rq)
        resp_qs.append(pq)
        solver_procs.append(p)

    solver_client = MultiSolverClient(req_qs, resp_qs)
