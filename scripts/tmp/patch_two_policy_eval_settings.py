...
# 1) two_policy base config: add validation solver response length
path = Path("/workspace/action_abstraction/verl/recipe/two_policy/config/two_policy_trainer.yaml")
text = path.read_text()
old = """  num_abstractions: 4
  num_solver_rollouts: 4
  validation_num_abstractions: 1
  validation_num_solver_rollouts: 1
  min_abstraction_chars: 32
"""
new = """  num_abstractions: 4
  num_solver_rollouts: 4
  validation_num_abstractions: 1
  validation_num_solver_rollouts: 1
...
# 2) two_policy trainer: pass validation-only solver response length through rollout meta_info
...
"""
if old not in text:
    raise SystemExit("rollout signature block not found")
...
"""
if old not in text:
    raise SystemExit("solver meta_info block not found")
text = text.replace(old, new, 1)
...
"""
if old not in text:
    raise SystemExit("validate header block not found")
text = text.replace(old, new, 1)
...
# 3) async agent-loop manager: honor per-batch temperature/top-p/top-k/response_length
path = Path("/workspace/action_abstraction/verl/verl/experimental/agent_loop/agent_loop.py")
text = path.read_text()
old = """        sampling_params = dict(
...
"""
new = """        response_length = int(batch.meta_info.get(\"response_length\", config.response_length))
...
"""
if old not in text:
    raise SystemExit("agent loop sampling block not found")
text = text.replace(old, new, 1)
...
"""
if old not in text:
    raise SystemExit("agent loop task launch block not found")
text = text.replace(old, new, 1)
...
"""
if old not in text:
    raise SystemExit("agent loop postprocess header block not found")
text = text.replace(old, new, 1)
for old, new in [
    ("max_length=self.rollout_config.response_length", "max_length=max_response_length"),
    ("pad_size = self.rollout_config.response_length - len(output.response_logprobs)", "pad_size = max_response_length - len(output.response_logprobs)"),
]:
    if old not in text:
        raise SystemExit(f"expected agent loop padding block not found: {old}")
    text = text.replace(old, new)
path.write_text(text)

# 4) full launchers: validation solver response length 32768
for path_str in [
...
"""
    if old not in text:
        raise SystemExit(f"length env block not found in {path}")
    text = text.replace(old, new, 1)
...
"""
    if old not in text:
        raise SystemExit(f"validation rollout block not found in {path}")
    path.write_text(text.replace(old, new, 1))
