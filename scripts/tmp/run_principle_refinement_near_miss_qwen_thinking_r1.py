...
OUT_DIR = REPO / 'outputs/2026-03-19/contrastive_abstraction_prompting/principle_refinement_near_miss_qwen_thinking_r1'
...
def main() -> None:
    ...
    md.append('- Thinking enabled: `true`')
    md.append('- Displayed refined principles have `<think>` blocks removed.')
    ...
    for ex, out in zip(examples, outputs):
        ...
        cleaned_refined = strip_think_blocks(raw_text).strip()
        ex['refined_principles_raw'] = raw_text
        ex['refined_principles_cleaned'] = cleaned_refined
        ex['raw_output_text'] = raw_text
        ex['contains_think'] = '<think>' in raw_text
        word_counts_initial.append(len(ex['initial_principles'].split()))
        word_counts_refined.append(len(cleaned_refined.split()))
        raw_word_counts.append(len(raw_text.split()))
        trace_counts.append(len(ex['correct_traces']))
        if '<think>' in raw_text:
            raw_with_think += 1
        if not cleaned_refined:
            cleaned_empty += 1

        md.append(f"## Dataset Index {ex['dataset_idx']} | Row {ex['row_id']}")
        md.append('')
        md.append(f"- Correct trace count: `{len(ex['correct_traces'])}`")
        md.append(f"- Raw output contains `<think>`: `{str('<think>' in raw_text).lower()}`")
        md.append('')
        md.append('Problem:')
        md.append('')
        ...
        md.append('```')
        md.append('')
        md.append('### Refined Principles (Thinking Cleaned)')
        md.append('')
        md.append('```text')
        md.append(cleaned_refined if cleaned_refined else '[empty after removing think blocks]')
        md.append('```')
        md.append('')
    ...
    report = {
        ...
        'mean_refined_words_cleaned': statistics.mean(word_counts_refined) if word_counts_refined else 0.0,
        'mean_refined_words_raw': statistics.mean(raw_word_counts) if raw_word_counts else 0.0,
        'raw_outputs_with_think': raw_with_think,
        'cleaned_empty_outputs': cleaned_empty,
