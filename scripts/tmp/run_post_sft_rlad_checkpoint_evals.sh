...
SWEEPS_ROOT=/workspace/action_abstraction/outputs/2026-03-22/contrastive_abstraction_prompting/rlad_non_regressed_half_epoch_checkpoint_sweeps
FAMILY_SUMMARY_ROOT="$SWEEPS_ROOT/family_summary"
mkdir -p "$SWEEPS_ROOT"
...
while kill -0 "$TRAIN_PID" 2>/dev/null; do
  date -u +"%Y-%m-%dT%H:%M:%SZ training still running"
  sleep 60
done

date -u +"%Y-%m-%dT%H:%M:%SZ training finished or exited"

mapfile -t CHECKPOINTS < <(find "$RUN_ROOT" -maxdepth 1 -type d -name 'checkpoint-*' | sort -V)
if [ ${#CHECKPOINTS[@]} -eq 0 ]; then
  echo "no checkpoints found under $RUN_ROOT" >&2
  exit 1
fi
...
for CKPT in "${CHECKPOINTS[@]}"; do
  ...
  for TEMP in 0.0 0.3 0.6 0.9; do
    ...
    echo "--- checkpoint-$STEP temp=$TEMP conditioned solving ---"
